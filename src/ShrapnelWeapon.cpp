    ...
    if(fire_intervale_clock.getElapsedTime().asMilliseconds() >= fire_intervale)
        {
            ...
            Engine::getInstance()->getMap()->addBullet(Bullet(user->getPosition(), angle, user->getmyTeam()));

            Engine::getInstance()->getMap()->addBullet(Bullet(user->getPosition(), aimangle2, user->getmyTeam()));

            Engine::getInstance()->getMap()->addBullet(Bullet(user->getPosition(), aimangle3, user->getmyTeam()));

            Engine::getInstance()->getMap()->addBullet(Bullet(user->getPosition(), aimangle4, user->getmyTeam()));

            Engine::getInstance()->getMap()->addBullet(Bullet(user->getPosition(), aimangle5, user->getmyTeam()));
        }
}
